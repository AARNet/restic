...
-e RESTIC_REPOSITORY=s3:https://s3.hostname.edu.au/test_restic_repo/ \
-v `pwd`:/backup -v `pwd`/logs:/var/log/restic restic
```
...
-e RCLONE_WEBDAV_URL=https://hostname.edu.au/webdav \
-e RESTIC_PASSWORD=super_secret_repository_password \
...
| -------- | -------- | -------- |
| RESTIC_REPOSITORY | `none` | The restic repository to use in the format of `s3:https://s3.hostname.edu.au/bucket_name` or `rclone::webdav:/path/for/repository` |
...
| Variable | Default | Purpose |
| -------- | -------- | -------- |
| RCLONE_WEBDAV_URL | `none` | The URL to the WebDAV endpoint (eg. `https://hostname.edu.au/webdav`)|
...
### Updating from official restic

To update this repository from the official restic, you can do the following.

**Note:** please only use [released tags](https://github.com/restic/restic/releases) to merge as the official restic `master` may contain unreleased code.

```
git clone https://github.com/AARNet/restic.git
cd restic
